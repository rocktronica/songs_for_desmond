...
void Display::drawAvatarFirst(int8_t x, int8_t y) {
  ...
  Arduboy2Base::fillRect(x + AVATAR_WIDTH, y, WIDTH - x - AVATAR_WIDTH,
                         AVATAR_HEIGHT, BLACK);
  Arduboy2Base::fillRect(0, y + AVATAR_HEIGHT, WIDTH,
                         HEIGHT - y - AVATAR_HEIGHT, BLACK);
}
...
void Display::drawProgressBar(int8_t x, int8_t y,
  ...
  drawPrettyTime(x + width - TIME_WIDTH, y, getSongLength(state.trackIndex));

  Arduboy2Base::drawRect(x + TIME_WIDTH + GAP_MIN, y, rectWidth,
                         PROGRESS_BAR_HEIGHT);
  Arduboy2Base::fillRect(x + TIME_WIDTH + GAP_MIN, y,
...
void Display::drawIntro(State &state) {
  ...
  if (animationFrame > INTRO_FRAMES) {
    tinyfont.setCursor(WIDTH - CHAR_SIZE * 4 - 1 * (4 - 1) - GAP_OUTER,
...
void Display::drawOperation(State &state, int8_t songsCount) {
  ...
  drawProgressBar(GAP_OUTER, HEIGHT - PROGRESS_BAR_HEIGHT - GAP_OUTER,
                  WIDTH - GAP_OUTER * 2, state);
}